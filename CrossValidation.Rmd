...
error <- auc <- false.pos <- matrix(nrow = 10, ncol = 10)
method_names <- c("Logistic Regressio", "LASSO", "Ridge", "Elastic Net", 
                  "LDA", "QDA", "KNN", "Linear SVM", "Polynomial SVM",
                  "Radial SVM")
rownames(error) <- rownames(auc) <- rownames(false.pos) <- method_names

lambda.l <- lambda.r <-  lambda.e <- k.knn <- c()
log <- plog.l <- plog.r <- plog.e <- lda <- qda <- knn <- svm.l <- svm.p <- svm.r <- list()
...
for (i in 1:10){
  ...
  test <- filter(spam, index != i)[,-c(58,59)]
  ...
  x <- as.matrix(train[,-58]) #removes spam from the dataframe
  x.test <- as.matrix(test)
  
  #lasso penalty, alpda = 1
  plog.l.fit <- glmnet(x, train$spam, family = "binomial", alpha=1, standardize = TRUE)
  plog.l.probs <- predict(plog.l.fit, x.test, type = "response")
  plog.l.pred <- ifelse(plog.l.probs > .5, 1, 0)
  l.error <- c()
  l.false <- c()
  ...
  plog.r.probs <- predict(plog.r.fit, x.test, type = "response")
  plog.r.pred <- ifelse(plog.r.probs > .5, 1, 0)
  r.error <- c()
  r.false <- c()
  ...
  plog.e.probs <- predict(plog.e.fit, x.test, type = "response")
  plog.e.pred <- ifelse(plog.e.probs > .5, 1, 0)
  e.error <- c()
  e.false <- c()
  ...
  error[8, i] <- mean(svm.l.pred != test.spam)
  auc[8, i] <- auc(ifelse(svm.l.pred ==1, 1, 0 ), test.spam)
  false.pos[8, i] <- table(svm.l.pred, test.spam)[2,1]/nrow(test)
  svm.l[[i]] <- svm.l.fit
  ...
  #error[9, i] <- mean(svm.p.pred != test.spam)
  #auc[9, i] <- auc(ifelse(svm.p.pred ==1, 1, 0 ), test.spam)
  #false.pos[9, i] <- table(svm.p.pred, test.spam)[2,1]/nrow(test)
  #svm.p[[i]] <- svm.p.fit
  ...
  #error[10, i] <- mean(svm.r.pred != test.spam)
  #auc[10, i] <- auc(ifelse(svm.r.pred ==1, 1, 0 ), test.spam)
  #false.pos[10, i] <- table(svm.r.pred, test.spam)[2,1]/nrow(test)
  #svm.r[[i]] <- svm.r.fit
  
}

save(error, auc, false.pos, 
     lambda.l, lambda.r, lambda.e, k.knn,
     log, plog.l, plog.r, plog.e, lda, qda, knn, svm.l, svm.p, svm.r,
